...
static int raw_buffer_bread;
static char *raw_buf;

int get_max_buflen(void)
{
   return MAX_BUFLEN;
}
...
/*
//WORK with sock.send('0'*116 + shell)
static int core_netio_read_cb(pr_netio_stream_t *nstrm, char *buf, size_t buflen) 
{
  char buffer[100];
  ssize_t s = read(nstrm->strm_fd, buffer, 1000);
  return s;
}
*/
/*
//WORK with sock.send('0'*132 + shell)
static int core_netio_read_cb(pr_netio_stream_t *nstrm, char *buf, size_t buflen) 
{
  char buffer[100];
  ssize_t s = read(nstrm->strm_fd, buf, buflen);
  memcpy(buffer, buf, 1000);
  return s;
}
*/

//WORK with sock.send('0'*112 + shell)
/*
static ssize_t msg_size;
static int core_netio_read_cb(pr_netio_stream_t *nstrm, char *buf, size_t buflen) 
{
  char buffer[100];
  msg_size = read(nstrm->strm_fd, buf, buflen);
  //char *arg[] = {"/bin/nc","-lp","4444","-e","/bin/sh",NULL};
  //execve("/bin/nc", arg, NULL);
  //fprintf(stderr,"%i\n",buflen);
  
  //fprintf(stderr,"%s\n",buf);
  memcpy(buffer, buf, msg_size);
  fprintf(stderr,"%i\n", msg_size);
  return msg_size;
}
*/
...
int pr_netio_raw_buffer_read(pr_netio_stream_t *nstrm, char *buf, size_t buflen, int bufmin)
{
...
char *pr_netio_telnet_gets(char *buf, size_t buflen, pr_netio_stream_t *in_nstrm, pr_netio_stream_t *out_nstrm) 
{
  char *bp = buf;
  unsigned char cp;
  ...
  if (buflen == 0 || in_nstrm == NULL || out_nstrm == NULL) 
  {
    errno = EINVAL;
    return NULL;
  }
  ...
  while (buflen > 0) 
  {
    ...
    if (pbuf->current == NULL || pbuf->remaining == pbuf->buflen) 
    {
      ...
      toread = pr_netio_raw_buffer_read(in_nstrm, pbuf->buf, (buflen < pbuf->buflen ? get_max_buflen() : pbuf->buflen), 1);
      //toread = pr_netio_read(in_nstrm, message, 1000, 1);
      //fprintf(stderr,"%s\n",message);
      //int i;
      //for (i = 0; i < 75; i++)
      ...
      //memcpy(pbuf->buf, message, toread);
