...
int pr_cmd_read(cmd_rec **res) 
{
  ...
  while (TRUE) 
  {
    ...
    if (pr_netio_telnet_gets(cmd_buf, cmd_bufsz, session.c->instrm, session.c->outstrm) == NULL) 
    {
      if (errno == E2BIG) 
      {
